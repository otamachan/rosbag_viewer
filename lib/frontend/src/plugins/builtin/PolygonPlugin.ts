import { PolygonDisplay } from "../../three/PolygonDisplay.ts";
import type { DisplayPlugin } from "../DisplayPlugin.ts";

export const PolygonPlugin: DisplayPlugin = {
  id: "polygon",
  canHandle: (type) => type === "geometry_msgs/PolygonStamped",
  createDisplay: (settings) => {
    const d = new PolygonDisplay(parseInt(String(settings.color ?? "#888888").replace("#", ""), 16));
    ...
    return {
      object: d.object,
      update: (msg) => d.updatePolygon(msg),
      applySettings: (s) => {
