import { PoseDisplay } from "../../three/PoseDisplay.ts";
import type { DisplayPlugin } from "../DisplayPlugin.ts";

export const PosePlugin: DisplayPlugin = {
  id: "pose",
  canHandle: (type) => type === "geometry_msgs/PoseStamped" || type === "geometry_msgs/PoseWithCovarianceStamped",
  createDisplay: (settings) => {
    const d = new PoseDisplay(
